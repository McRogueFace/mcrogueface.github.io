...
- Computing FOV with `grid.compute_fov()`
- Using `ColorLayer` to visualize visibility states
- Tracking three visibility states: visible, discovered, unknown
- Updating FOV when the player moves
- Setting the `transparent` property for walls
...
- Alpha 0: Completely transparent (tile shows through)
- Alpha 180: Partially transparent (dimmed effect)
- Alpha 255: Completely opaque (tile hidden)
...
- `player_x, player_y`: The observer's position
- `FOV_RADIUS`: How far the player can see
- `mcrfpy.FOV.SHADOW`: The shadowcasting algorithm (recommended)
...
- Spawning enemy entities in rooms
- Making enemies only visible when in FOV
- Basic entity data structures
- Blocking movement with other entities

[Continue to Part 5: Placing Enemies](part_05_enemies.md)
...
- **FOV layer**: `grid.add_layer("color", z_index=-1)`
- **Exploration tracking**: `explored` 2D array with `mark_explored()` and `is_explored()`
- **FOV computation**: `grid.compute_fov()` and `grid.is_in_fov()`
- **Visibility colors**: `COLOR_VISIBLE`, `COLOR_DISCOVERED`, `COLOR_UNKNOWN`
- **Transparent property**: Set on all tiles for proper FOV calculation
