...
- `xp_to_next_level`: Computed threshold for next level
...
1. **Exponential XP curve**: Make higher levels require more XP:
   ```python
   @property
   def xp_to_next_level(self) -> int:
...
2. **Stat choice on level up**: Let player choose their stat increase:
   ```python
   class GameMode(Enum):
       NORMAL = "normal"
       TARGETING = "targeting"
       LEVEL_UP = "level_up"  # New mode

   def show_level_up_menu():
       game_mode = GameMode.LEVEL_UP
       message_log.add("Level up! Press 1:+HP, 2:+Attack, 3:+Defense")
   ```

3. **XP from exploration**: Award XP for discovering new tiles

4. **Level-based enemy scaling**: Enemies get stronger as you level:
   ```python
   def get_enemy_stats(base_stats, player_level):
       scaling = 1 + (player_level - 1) * 0.1
       return {
           "hp": int(base_stats["hp"] * scaling),
           "attack": int(base_stats["attack"] * scaling)
       }
   ```

5. **Skills or abilities**: Unlock new abilities at certain levels
...
1. **Forgetting to save XP**: Update serialization when adding new fields

2. **XP awarded after removal**: Award XP before removing the dead entity

3. **Integer division**: Use proper math for XP calculations

4. **Not updating UI**: Call `update_ui()` after XP changes

5. **Level 0 bugs**: Make sure level starts at 1, not 0

6. **Missing default values**: Use `.get()` with defaults when loading old saves
...
[Continue to Part 13: Equipment System](part_13_equipment.md)
